...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'taco-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('taco').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('taco')[service]);
